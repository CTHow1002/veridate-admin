...
export type UserModerationAction = "update-ban" | "unban" | "clear-warning";
...
export async function getModeratedUsers(): Promise<ModeratedUser[]> {
  const profiles = await supabaseRequest<ModeratedProfileRow[]>(
    "/rest/v1/profiles?or=(is_banned.eq.true,warned_at.not.is.null,warning_message.not.is.null)&select=id,full_name,verification_status,is_banned,ban_until,ban_message,ban_details,warning_message,warning_details,warned_at"
  );
